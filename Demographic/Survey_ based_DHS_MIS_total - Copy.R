...
filename          <- paste("./Input_data/Household surveys/DHS/", name, ".sav", sep ="")

raw_data          <- read.spss(filename, to.data.frame = TRUE) 

survey_data <- raw_data %>%
  select(c("HHID","HV005", "HV021","HV022", "HV024", "HV104", "HV105"))


# Bring in complex survey format ------------------------------------------

survey <- svydesign(id      = survey_data$HV021,         # Primary sampling unit
                    strata  = survey_data$HV022,         # Strata used for standard errors
                    weights = survey_data$HV005/1000000, # Household sample weight
                    data    = survey_data)               # Data set used

# estimate share of sex/age groups by region

total <- as.data.frame(svytable(~HV105+HV104, survey))
total <- cbind("Total", total)


names(total)<- c("region", "age", "sex", "frequency")
...
data$sex <- ifelse(data$sex == "Homme" | data$sex == "Masculin" | data$sex == "Masculino" | data$sex == "Male", "Male",
                   ifelse(data$sex == "Femme" | data$sex == "Feminin" | data$sex == "Femenino" | data$sex == "Female" | data$sex == "Feminino", "Female", 
                          ifelse(data$sex == "Missing" | data$sex == "DK", NA, data$sex)))

data$age <- ifelse(data$age == "Don't know", NA, data$age)
...
write.xlsx(as.data.frame(population_estimate), "./results/estimate_5year.xlsx", row.names = F)

# visualize with pop pyramid ----------------------------------------------


regions <- unique(population_estimate$region)

population_estimate <- population_estimate %>%  
  mutate(age.group = factor(x = age.group, levels = unique(age.group)))%>%
  filter(region != "Borkou/Tibesti")
